...
export async function POST(request: NextRequest) {
  ...
  text_urgency_score: (blockRequest as any).text_urgency_score ?? (blockRequest.description ? 7 : 5),
  description: (blockRequest as any).description ?? blockRequest.work_type ?? "",
}),
      cache: 'no-store',
    })
